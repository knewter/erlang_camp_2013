%%% @author Josh Adams <josh@penis.com>
...
%% @doc Insert a key and a value into the cache
-spec insert(Key::atom(), Value::term()) -> ok.
insert(Key, Value) ->
    gen_server:cast(?SERVER, {insert, {Key, Value}}).

%% @doc Lookup a value based on its key
-spec lookup(Key::atom()) -> term().
lookup(Key) ->
